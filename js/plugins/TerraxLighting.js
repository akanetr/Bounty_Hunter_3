...
Imported.TerraxLighting = true;
	...
	var Terrax_tint_speed = 160;
	var Terrax_tint_target = '#444444';
...
(function() {
	...
	var mogdebug = false;
	var terrax_tint_speed_old = 160;
	var terrax_tint_target_old = '#444444'
	var tileglow = 0;
		...
		if (map_id != oldmap) {
			...
			if ($gameVariables.GetMog() == true) {
				...
				if (searchdaynight.search('mogtime') >= 0) {
					...
					$gameVariables.SetTintTarget('#444444');
					$gameVariables.SetTint('#444444');
				}
			}
			...
			if ($gameVariables.GetStopScript() == false) {
				...
				if ($gameVariables.GetScriptActive() == true && $gameMap.mapId() >= 0) {
					...
					if (darkenscreen == true) {
						...
						if (firstrun === true) {
							Terrax_tint_speed = 160;
							Terrax_tint_target = '#444444';
							terrax_tint_speed_old = 160;
							terrax_tint_target_old = '#444444';
						...
						this._maskBitmap.fillRect(0, 0, maxX + 20, maxY, '#444444');
						...
						if (iplayer_radius > 0) {
							if (playerflashlight == true) {
								this._maskBitmap.radialgradientFillRect2(x1, y1, 20, iplayer_radius, playercolor, '#444444', pd, flashlightlength, flashlightwidth);
							}
	...
	Bitmap.prototype.radialgradientFillRect = function(x1, y1, r1, r2, color1, color2, flicker, brightness, direction) {
		...
		if (clip == false) {
			...
			var context = this._context;
			var grad;
			var wait = Math.floor((Math.random() * 8) + 10);
