 ...
 *
 * Script Call lines
 *  this._chaseRange = x       x の距離内に入った時、イベントがプレイヤーを追いかけます
 *  this._fleeRange = x        x の距離内に入った時、イベントはプレイヤーから逃げます
 *  this._chaseSpeed = x       追いかける時のイベントの速度 x を決定します
 *  this._fleeSpeed = x        逃げる時のイベントの速度 x を決定します
 *  this._sightLock = x        イベントは、フレーム数 x だけ追跡/逃走を行います
 *  this._seePlayer = true     イベントはプレイヤーを認識する必要があります
 *  this._seePlayer = false    イベントはプレイヤーを認識する必要がありません
 *  this._alertBalloon = x     プレイヤーが認識されると、x というフキダシを表示します
 *  this._alertSound = x       プレイヤーが認識されると、x というサウンドを再生します
 *  this._alertSoundVol = x    プレイヤーが認識されたときに再生される音の音量を指定します
 *  this._alertSoundPitch = x  プレイヤーが認識されたときに再生される音のピッチを指定します
 *  this._alertSoundPan = x    プレイヤーが認識されたときに再生される音のパンを指定します
 *  this._alertCommonEvent = x プレイヤーが認識されると、x というイベントを再生します
 *  this._returnAfter = true   イベントを元の位置に戻します
 *  this._returnAfter = false  追跡/逃走が終わってもイベントはそこにとどまります
 *  this._returnWait = x       追跡/逃走が終わったあと x の間待機します
 *
