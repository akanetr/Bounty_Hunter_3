...
//=============================================================================
	...
	Window_EnemyBookPercent.prototype.refresh = function() {
           ...
           if ($gameSwitches.value(153)) tehai=11;
           ...
           this.drawText(hokakus, w1-70, 0, w1, 'right');

           this.changeTextColor(this.textColor(10));
           this.drawText("/", w1-55, 0, w1, 'right');

           this.resetTextColor();

           this.drawText(tehai, w1-25, 0, w1, 'right');

           this.changeTextColor(this.textColor(10));
           this.drawText("人", w1, 0, w1, 'right');
           this.resetTextColor();


           //this.drawText(hokakus + "/" + tehai + "人", w1, 0, w1, 'right');
	}
...
//=============================================================================
	...
	Window_EnemyBookIndex.prototype.initialize = function(x, y) {
		...
		Window_Selectable.prototype.initialize.call(this, x, y, width-100, height);
		//this.refresh();
	}
		...
		if (this.enemy) {
			...
			for (var i = 1; i <= 10; i++) {
			   ...
			   var gameEnemy = new Game_Enemy(i,0,0);
                        ...
                        gameEnemy = new Game_Enemy(11,0,0);
                        if (i==11 && $gameSwitches.value(153)) this._list.push(gameEnemy);
...
//=============================================================================
	...
	Window_EnemyBookStatus.prototype.initialize = function(x, y, width, height) {
		Window_Base.prototype.initialize.call(this, x, y, width-300, height+104);
		this._defaultX = x;
		this._enemy = null;
		this._enemySprite = new Sprite();
		this._enemySprite.anchor.x = 1;
		this._enemySprite.anchor.y = 1;
		this._enemySprite.x = 510;
		this._enemySprite.y = 460;
	...
	Window_EnemyBookStatus.prototype.refresh = function() {
		...
		if ($gameSystem.isSideView()) {
			bitmap = ImageManager.loadSvEnemy(name, hue);
                   ...
                   if (name=="手配書-レッドアカ" && !$gameSwitches.value(141)) {
		      bitmap = ImageManager.loadEnemy("手配書-レッドアカ", hue);
$gameVariables.setValue(10,999);
		   } else if (name=="手配書-レッドアカ" && $gameSwitches.value(141)) {
		      bitmap = ImageManager.loadEnemy("手配書-レッドアカ-捕獲", hue);

		   } else if (name=="手配書-ラ・ビアード" && !$gameSwitches.value(142)) {
		      bitmap = ImageManager.loadEnemy("手配書-ラ・ビアード", hue);
$gameVariables.setValue(10,111);
		   } else if (name=="手配書-ラ・ビアード" && $gameSwitches.value(142)) {
		      bitmap = ImageManager.loadEnemy("手配書-ラ・ビアード-捕獲", hue);

		   } else if (name=="手配書-ゴーグルマン" && !$gameSwitches.value(143)) {
		      bitmap = ImageManager.loadEnemy("手配書-ゴーグルマン", hue);
		   } else if (name=="手配書-ゴーグルマン" && $gameSwitches.value(143)) {
		      bitmap = ImageManager.loadEnemy("手配書-ゴーグルマン-捕獲", hue);

		   } else if (name=="手配書-ナルオウ" && !$gameSwitches.value(144)) {
		      bitmap = ImageManager.loadEnemy("手配書-ナルオウ", hue);
$gameVariables.setValue(10,222);
		   } else if (name=="手配書-ナルオウ" && $gameSwitches.value(144)) {
		      bitmap = ImageManager.loadEnemy("手配書-ナルオウ-捕獲", hue);

		   } else if (name=="手配書-クイックダーク" && !$gameSwitches.value(145)) {
		      bitmap = ImageManager.loadEnemy("手配書-クイックダーク", hue);
		   } else if (name=="手配書-クイックダーク" && $gameSwitches.value(145)) {
		      bitmap = ImageManager.loadEnemy("手配書-クイックダーク-捕獲", hue);

		   } else if (name=="手配書-ロングモンキー" && !$gameSwitches.value(146)) {
		      bitmap = ImageManager.loadEnemy("手配書-ロングモンキー", hue);
		   } else if (name=="手配書-ロングモンキー" && $gameSwitches.value(146)) {
		      bitmap = ImageManager.loadEnemy("手配書-ロングモンキー-捕獲", hue);

		   } else if (name=="手配書-ピンクウェンディ" && !$gameSwitches.value(147)) {
		      bitmap = ImageManager.loadEnemy("手配書-ピンクウェンディ", hue);
		   } else if (name=="手配書-ピンクウェンディ" && $gameSwitches.value(147)) {
		      bitmap = ImageManager.loadEnemy("手配書-ピンクウェンディ-捕獲", hue);

		   } else if (name=="手配書-バイパー" && !$gameSwitches.value(148)) {
		      bitmap = ImageManager.loadEnemy("手配書-バイパー", hue);
		   } else if (name=="手配書-バイパー" && $gameSwitches.value(148)) {
		      bitmap = ImageManager.loadEnemy("手配書-バイパー-捕獲", hue);

		   } else if (name=="手配書-ミルクタンク" && !$gameSwitches.value(149)) {
		      bitmap = ImageManager.loadEnemy("手配書-ミルクタンク", hue);
		   } else if (name=="手配書-ミルクタンク" && $gameSwitches.value(149)) {
		      bitmap = ImageManager.loadEnemy("手配書-ミルクタンク-捕獲", hue);

		   } else if (name=="手配書-アイスモスキート" && !$gameSwitches.value(150)) {
		      bitmap = ImageManager.loadEnemy("手配書-アイスモスキート", hue);
		   } else if (name=="手配書-アイスモスキート" && $gameSwitches.value(150)) {
		      bitmap = ImageManager.loadEnemy("手配書-アイスモスキート-捕獲", hue);

		   } else if (name=="手配書-シュウキ" && !$gameSwitches.value(151)) {
		      bitmap = ImageManager.loadEnemy("手配書-シュウキ", hue);
		   } else if (name=="手配書-シュウキ" && $gameSwitches.value(151)) {
		      bitmap = ImageManager.loadEnemy("手配書-シュウキ-捕獲", hue);
		...
		//this.resetTextColor();
		//this.drawText(enemy.name(), x+280, y, columnWidth);
		...
		for (var i = 0; i < 8; i++) {
			if (dispParameters[i]) {
				if (i == 0 && $gameTroop.inBattle() && ($gameSystem.isShowCurrentEnemysStatus() || this.isCheck)) {
					...
					this.resetTextColor();
					if (!isUnknownEnemy) {
						this.drawText(zengo + enemy.param(i), x + 160, y, 60, 'right');
					} else {
						this.drawText(UnknownData, x + 140, y, 80);
					}
				}
		...
		if (!isUnknownEnemy && DispDescribe) {
			this.drawTextEx(dataEnemy.meta.desc1, x, (y+230) + lineHeight * 0);
			this.drawTextEx(dataEnemy.meta.desc2, x, (y+230) + lineHeight * 1);
			this.drawTextEx(dataEnemy.meta.desc3, x, (y+230) + lineHeight * 2);
			this.drawTextEx(dataEnemy.meta.desc4, x, (y+230) + lineHeight * 3);
			this.drawTextEx(dataEnemy.meta.desc5, x, (y+230) + lineHeight * 4);
		}
	};
