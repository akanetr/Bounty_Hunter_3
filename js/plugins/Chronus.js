...
(function() {
    ...
    Scene_Map.prototype.onMapLoaded = function() {
        var chronus = $gameSystem.chronus();
        chronus.updateWeatherType();
        chronus.updateWeatherPower();
        chronus.refreshTint(true);
    ...
    Window_Chronus.prototype.initialize = function() {
        ...
        _Window_Chronus_initialize.call(this, 0, 0, width + pad * 2, 14 * 2 + pad * 2);
        this.padding  = pad;
    ...
    Window_Chronus.prototype.refresh = function() {
        this.contents.clear();
        //変更　this.contents.drawText(this.chronus().getDateFormat(1),
        //変更　    0, 0, this.contents.width, this.contents.fontSize, 'left');
    ...
    Game_Chronus.prototype.setTint = function(timeId, swift) {
       ...
       if ($gameVariables.value(180) == 0) {
           $gameScreen.startTint(tone, swift ? 0 : this.getEffectDuration());
        }

       //変更追加　マップ上の明かりを時間帯によって変更、非戦闘時
     //  if (($gameVariables.value(1) != $gameVariables.value(33)) && !$gameSwitches.value(21)) {
     //     $gameVariables.setValue(33,$gameVariables.value(1));
    ...
    Game_Chronus.prototype.setWeatherPower = function(value) {
        this._weatherPower = value.clamp(0, 50);
    };
    ...
    Game_Chronus.prototype.setScreenWeather = function(swift) {
        if (!$gameSwitches.value(219)) {
           ...
           //変更　屋外に居る時　雨が降ってきた場合は雨の効果音
           if ($gameVariables.value(5) != 0 && $gameVariables.value(180) == 0 && !$gameSwitches.value(66)) {
              $gameSwitches.setValue(66,true);
              AudioManager.playBgs({"name":"Rain","volume":40,"pitch":100,"pan":0});
           } else if ($gameVariables.value(5) == 0) {
              $gameSwitches.setValue(66,false);
              AudioManager.fadeOutBgs(5)
           }
    ...
    //======================================
       ...
       //変更追加　マップ上の明かりを時間帯によって変更、非戦闘時 （1時間毎）
       if (($gameVariables.value(1) != $gameVariables.value(33)) && !$gameSwitches.value(21)) {
          $gameVariables.setValue(33,$gameVariables.value(1));
          $gameSwitches.setValue(1,true);
       }
       ...
       //変更追加　宿のチェックアウト
       if (($gameVariables.value(1) == $gameVariables.value(178)) && $gameSwitches.value(73)) {
          $gameSwitches.setValue(72,true);
       }

       //変更追加　日付変更チェック
       if ($gameVariables.value(176)==$gameVariables.value(175)) {
          $gameSwitches.setValue(140,true);
       }

       //変更追加　バイパー親密度　0が条件
       if ($gameVariables.value(660) == 0) {
          //変更追加　バイパー強盗中チェック
          if ($gameVariables.value(628)!=0 && !$gameSwitches.value(21) && !$gameSwitches.value(135) && !$gameSwitches.value(194) && (($gameVariables.value(628)<=5 && $gameVariables.value(1)==1) || ($gameVariables.value(628)==6 && $gameVariables.value(1)==13))) {
             $gameSwitches.setValue(195,true);
          }
          //変更追加　バイパー強盗終了チェック
          if ($gameVariables.value(628)!=0 && !$gameSwitches.value(148) && !$gameSwitches.value(192) && $gameSwitches.value(194) &&  !$gameSwitches.value(432) && (($gameVariables.value(628)<=5 && $gameVariables.value(1)>=2) || ($gameVariables.value(628)==6 && $gameVariables.value(1)>=14) || ($gameVariables.value(1)==1 && $gameVariables.value(2)>=11) || ($gameVariables.value(1)==13 && $gameVariables.value(2)>=11))) {
             $gameSwitches.setValue(193,true);
          }
          //変更追加　2時以降バイパー強盗中フラグオフ
          if (($gameVariables.value(628)<=5 && $gameVariables.value(1)>=2) || ($gameVariables.value(628)==6 && $gameVariables.value(1)>=14)) {
             $gameSwitches.setValue(192,false);
             $gameSwitches.setValue(194,false);
          }
       }

       //変更追加　ラーダ調査から帰還
       if (($gameVariables.value(1) == $gameVariables.value(197)) && $gameSwitches.value(217)) {
          $gameSwitches.setValue(216,true);
       }

       $gameVariables.setValue(174,$gameVariables.value(174)+1);
       //変更追加　累計時間10以上でもよおし度100以下なら
       if ($gameVariables.value(174) >= 50 && $gameVariables.value(36) < 100) {
       ...
       if (!$gameSwitches.value(160)) {
          //立ち絵表示中で撫でていない、非戦闘中
          if ($gameSwitches.value(9) && !$gameSwitches.value(16) && !$gameSwitches.value(21)) {
              ...
              if ($gamePlayer.isMoving()) {
                 ...
                 if ($gameVariables.value(32) >= moyo_count) {
                    moyo_count = 0;
                    //立ち絵フラグ全オフの場合のみ選択へ
                    if(!$gameSwitches.value(11) && !$gameSwitches.value(12) && !$gameSwitches.value(13) && !$gameSwitches.value(14)){
