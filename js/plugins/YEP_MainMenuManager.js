...
Window_MenuCommand.prototype.createCommand = function(i) {
    ...
    //変更　病院バイト中は装備使用不可
    //if (i === 20 && $gameSwitches.value(169)) enabled = false;

    var ext = eval(Yanfly.MMM.Ext[i]);

    //変更 オリジナルコマンド箇所を英語化
    if (i === 17 && $gameSwitches.value(115) == false) {
        this.addCommand('Reload', symbol, enabled, ext);
    } else if (i === 52 && $gameSwitches.value(115) == false) {
        this.addCommand('Wanted', symbol, enabled, ext);
    } else if (i === 55 && $gameSwitches.value(115) == false) {
        this.addCommand('Standing picture', symbol, enabled, ext);
    } else if (i === 56 && $gameSwitches.value(115) == false) {
        this.addCommand('Map', symbol, enabled, ext);
    } else if (i === 57 && $gameSwitches.value(115) == false) {
        this.addCommand('Masturbation', symbol, enabled, ext);
    } else if (i === 58 && $gameSwitches.value(115) == false) {
        this.addCommand('Reminiscence', symbol, enabled, ext);
    } else {
