...
Window_Base.prototype.drawActorMp = function(actor, x, y, width) {
    ...
    this.drawGauge(x, y, width, ($gameVariables.value(37) / 100), color1, color2);
    this.changeTextColor(this.systemColor());
    //日本語
    if ($gameSwitches.value(115)) {
       this.drawText("興奮", x, y, 44);
    } else {
       this.drawText("Excitement", x, y, 44);
    }
    this.changeTextColor(this.mpColor(actor));
    this.drawText($gameVariables.value(37), x + width - 65, y, 64, 'right');
};
...
Window_Help.prototype.initialize = function(numLines) {
    ...
    if ($gameSwitches.value(18)==true) {
       Window_Base.prototype.initialize.call(this, 0, 0, width-304, height);
...
Window_MenuStatus.prototype.initialize = function(x, y) {
    var width = this.windowWidth();
    var height = this.windowHeight();
    Window_Selectable.prototype.initialize.call(this, x, y, width-304, height);
...
Window_Status.prototype.initialize = function() {
    var width = Graphics.boxWidth-304;
...
Window_Status.prototype.drawBasicInfo = function(x, y) {
    ...
    this.drawActorIcons(this._actor, x+80, (y + lineHeight * 0)-16);

    this.drawActorHp(this._actor, x, (y + lineHeight * 2)-56);
    //変更　ラーダの場合非表示
    if (!$gameSwitches.value(160)) {
       this.drawActorMp(this._actor, x, (y + lineHeight * 3)-56);
       this.drawActorTp(this._actor, x, (y + lineHeight * 4)-56);
       this.drawActorbb(this._actor, x, (y + lineHeight * 5)-56);
    }
};
...
Window_Status.prototype.drawEquipments = function(x, y) {
    ...
    for (var i = 0; i < count; i++) {
        this.drawItemName(equips[i], x, y+6 + this.lineHeight() * i);
    }
};
...
Window_Message.prototype.windowWidth = function() {
    ...
    if ($gameSwitches.value(16)) {
       return Graphics.boxWidth - $gameVariables.value(695);
    } else {
       return Graphics.boxWidth;
    }
};
...
Window_Message.prototype.updatePlacement = function() {
    ...
    //変更追加　メッセージウィンドウ高さ調整 立ち絵なで中スイッチオンの場合
　　if (this._positionType === 2 && $gameSwitches.value(16)) this.y -= 450;
　　if ($gameSwitches.value(16)) {
       this.x = $gameVariables.value(696);
       this.y = $gameVariables.value(697);
    }
　　if ($gameSwitches.value(45)) this.x = 0;
...
Window_TitleCommand.prototype.updatePlacement = function() {
    ...
    this.x = 210; //(Graphics.boxWidth - this.width) / 2;

    this.y = 410; //Graphics.boxHeight - this.height - 96;
